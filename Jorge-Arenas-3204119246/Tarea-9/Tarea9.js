...
function obtenerSelecciones() {
  ...
  function obtenerJugadoresPorSeleccion(codigoPais) {
    return new Promise((resolve) => {
      setTimeout(() => {
        const jugadoresDeSeleccion = jugadores.filter(jugador => jugador.codigoPais === codigoPais);
        resolve(jugadoresDeSeleccion);
      }, 5000);
    });
  }
  obtenerSelecciones()
    ...
    .then(jugadoresPorSeleccion => {
      jugadoresPorSeleccion.forEach((jugadores, index) => {
        console.log(`Jugadores de la selección ${selecciones[index].nombre}:`, jugadores);
      });
    })
    .catch(error => {
      console.error(error);
